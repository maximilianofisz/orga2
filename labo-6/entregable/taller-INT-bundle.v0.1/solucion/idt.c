...
*/
/* COMPLETAR: Dado un número de interrupción asigna a `idt` la entrada
 * correspondiente con nivel 0 (kernel level). */
#define IDT_ENTRY0(numero)                                                     \
  idt[numero] = (idt_entry_t) {                                                \
    .offset_31_16 = HIGH_16_BITS(&_isr##numero), /* Parte alta de la dirección de la ISR */ \
    .offset_15_0 = LOW_16_BITS(&_isr##numero),   /* Parte baja de la dirección de la ISR */ \
    .segsel = GDT_CODE_0_SEL,                      /* Selector de segmento que apunta al segmento de código del kernel en la GDT */ \
    .type = INTERRUPT_GATE_TYPE,                /* Tipo 0xE indica una Interrupt Gate de 32 bits */ \
    .dpl = 0,                                     /* Nivel de privilegio 0 (nivel de kernel) */ \
    .present = 1                                  /* Marca la entrada como presente en la IDT */ \
  }

/* COMPLETAR: Dado un número de interrupción asigna a `idt` la entrada
 * correspondiente con nivel 3 (user level). */
#define IDT_ENTRY3(numero)                                                     \
  idt[numero] = (idt_entry_t) {                                                \
    .offset_31_16 = HIGH_16_BITS(&_isr##numero), /* Parte alta de la dirección de la ISR */ \
    .offset_15_0 = LOW_16_BITS(&_isr##numero),   /* Parte baja de la dirección de la ISR */ \
    .segsel = GDT_CODE_3_SEL,                      /* Selector de segmento igual que para nivel 0, apuntando al segmento de código del kernel */ \
    .type = INTERRUPT_GATE_TYPE,               /* Tipo 0xE indica una Interrupt Gate de 32 bits */ \
    .dpl = 3,                                     /* Nivel de privilegio 3 (nivel de usuario), permitiendo que el código de usuario dispare esta interrupción */ \
    .present = 1                                  /* Marca la entrada como presente en la IDT */ \
  }
...
void idt_init() {
  ...
  IDT_ENTRY3(88);
  IDT_ENTRY3(98);
  // aca quedo quiza medio algo fishy que pareciera que se quiere probar interrups desde dpl 3, pero caemos en un error #13 de cambio de privilegio
  // si usamos entry3. me parece raro tambien el hecho de que estas ints las estoy llamando desde kernel.asm asi que yo esperaria que sean de dpl 0
  // usando entry0 funciona!
}
